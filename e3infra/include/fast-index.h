...
/*
*findex_2_4 for <mac,fwd-entry> in L2 bridging forwarding
*/
#define FINDEX_2_4_TAGS_LENGTH 48
#if defined(__AVX2__)

struct findex_2_4_tag_block{
    uint16_t tag1[16];
    uint16_t tag2[16];
}__attribute__((packed));
#define MAX_2_4_TAGS_NR_OF_ENTRY 3
...
#define MAX_2_4_TAGS_NR_OF_ENTRY 6

#else 
#error "please make sure target machine at least supports SSE4.1 instruction set"
#endif

struct findex_2_4_entry{
    /*32-bytes header room only*/
    e3_bitmap tag_avail;
    struct findex_2_4_entry * next_entry;
    
    __attribute__((aligned(32))) 
        struct findex_2_4_tag_block 
            tags[MAX_2_4_TAGS_NR_OF_ENTRY];
    struct{
        uint64_t leaf_fwd_entry_as_64;
        uint64_t ts_last_updated;
    }_values[FINDEX_2_4_TAGS_LENGTH];

    __attribute__((aligned(64)))
        struct rcu_head rcu;
};

#define COPY_TO_FINDEX_2_2_VALUE(entry,index,v1_u64,v2_u64) {\
    (entry)->_values[(index)].leaf_fwd_entry_as_64=(v1_u64); \
    (entry)->_values[(index)].ts_last_updated=(v2_u64); \
}

#define COPY_FROM_FINDEX_2_2_VALUE(entry,index,v1_u64,v2_u64) {\
    (v1_u64)=(entry)->_values[(index)].leaf_fwd_entry_as_64; \
    (v2_u64)=(entry)->_values[(index)].ts_last_updated; \
}

struct findex_2_4_base{
    struct findex_2_4_entry * next;
};

struct findex_2_4_key{
    uint16_t key_index;
    uint16_t tag1;
    uint16_t tag2;
    struct {
        ...
        int update)
{
    ...
    int ret=-1;
    ...
    #if defined(__AVX2__)
        __m256i ymm_tag1,ymm_tag2;
        __m256i ymm_cmp_target1,ymm_cmp_target2;
        __m256i ymm_cmp_rc1,ymm_cmp_rc2;
        __m256i ymm_cmp_rc;
        __m256i ymm_shuffle_mask;
        ...
        #define PERMUTATION_MASK (0|(2<<2)|(1<<4)|(3<<6))
    #elif defined(__SSE4_1__)
        __m128i xmm_tag1,xmm_tag2;
        __m128i xmm_target1,xmm_target2;
        __m128i xmm_cmp_rc1,xmm_cmp_rc2;
        __m128i xmm_cmp_rc;
        __m128i xmm_shuffle_mask;
    ...
    pentry=rcu_dereference(pentry->next_entry)){
        #if defined(__AVX2__)
            for(idx=0,index_base=0;(ret)&&(idx<MAX_2_4_TAGS_NR_OF_ENTRY);idx++,index_base+=16){
                ymm_tag1=_mm256_load_si256((__m256i*)pentry->tags[idx].tag1);
                ymm_tag2=_mm256_load_si256((__m256i*)pentry->tags[idx].tag2);
                ymm_cmp_rc1=_mm256_cmpeq_epi16(ymm_tag1,ymm_cmp_target1);
                ymm_cmp_rc2=_mm256_cmpeq_epi16(ymm_tag2,ymm_cmp_target2);
        ...
        #elif defined (__SSE4_1__)
            for(idx=0,index_base=0;(ret)&&(idx<MAX_2_4_TAGS_NR_OF_ENTRY);idx++,index_base+=8){
                xmm_tag1=_mm_load_si128((__m128i *)pentry->tags[idx].tag1);
                xmm_tag2=_mm_load_si128((__m128i *)pentry->tags[idx].tag2);
                xmm_cmp_rc1=_mm_cmpeq_epi16(xmm_tag1,xmm_target1);
                xmm_cmp_rc2=_mm_cmpeq_epi16(xmm_tag2,xmm_target2);
